...
def elastic_search(config, env, ops_bucket_name, region):
...
def create_s3databucket(config, env, ops_bucket_name, region):
    ...
    stack_name = f"kf-artemis-s3-bucket-data-{env}-{config.NamingSuffix}"
    bucket = ops_bucket_name
...
def main():
    ...
    if eS_instance_count = 1:
        elastic_search(config, environment, ops_bucket_name, region)
